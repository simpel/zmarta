...
import {AlertTriangle, Loader} from 'tabler-icons-react';
import {convertToCurrency, getMonthlyCost} from '../Utils/Utils';
import {TBank, TResponseData} from './TBank';
...
const Result = () => {
	...
	const data = axios.data as TResponseData;
	const isLoading = axios.loading;
	const {error} = axios;
	...
	const renderBanks = () => {
		const dom: Array<ReactElement<HTMLLIElement>> = [];

		if (data) {
			for (const [index, bank] of data.availableBanks.entries()) {
				const cost = getMonthlyCost(
					Number(amount),
					Number(repaymentYears),
					bank.interrest,
				);

				dom.push(
					<li
						className="flex items-center justify-between w-full p-4 my-4 border shadow-xl"
						key={`bank_${index}`}
					>
						<div>
	...
	return (
		<div className="container max-w-3xl mx-auto">
			<h1 className="text-2xl font-extrabold text-center font-sharp">
				Grattis! Vi kan erbjuda dig lån hos {data.availableBanks.length} av{' '}
				{data.totalNumOfBanks} banker!
			</h1>
			<ul className="w-full">{renderBanks()}</ul>
			<button
				type="button"
				onClick={() => {
					navigator('/');
